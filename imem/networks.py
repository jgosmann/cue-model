...
def OneHotCounter(n, **kwargs):
    with nengo.Network(seed=73, **kwargs) as net:
        with nengo.presets.ThresholdingEnsembles(0.):
            ...
            net.inhibit_threshold = nengo.networks.EnsembleArray(20, n)
        ...
        nengo.Connection(
            net.state.add_output('const', lambda x: 1.2), net.state.input,
            synapse=0.1)

        nengo.Connection(
            net.bias, net.inhibit_threshold.input,
            transform=-0.6 * np.ones((n, 1)))
        nengo.Connection(net.state.output, net.inhibit_threshold.input)
        nengo.Connection(
            net.inhibit_threshold.add_output('heaviside', lambda x: x > 0.),
            net.state.input, transform=-2. * np.roll(np.eye(n), 1, axis=1))
        nengo.Connection(
            net.inhibit_threshold.heaviside,
            net.state.input, transform=-2. * np.roll(np.eye(n), -2, axis=1))

        # nengo.Connection(
            # net.bias, net.advance_threshold.input, transform=-np.ones((n, 1)))
        nengo.Connection(
